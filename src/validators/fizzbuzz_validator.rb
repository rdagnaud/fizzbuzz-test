...
=end
  def self.validate_fizzbuzz(payload)
    ...
    raise BadRequestError, "Missing keys #{missing_keys.join(', ')}" if missing_keys.any?

    integer_parameters = payload.values_at(:int1, :int2, :limit)

    # As fizzbuzz logic starts at 1, limit should be 1 or greater
    raise BadRequestError, "Integers must be 1 or greater" if integer_parameters.any?{ |n| n.to_i < 1 }
    ...
    raise BadRequestError, "Integers must be 1000000 or smaller" if integer_parameters.any?{ |n| n.to_i > 1000000 }

    string_parameters = payload.values_at(:str1, :str2)

    raise BadRequestError, "Strings must not be empty" if string_parameters.any?{ |s| s.empty? }

    # Adding an arbitrary upper limit to string size
    raise BadRequestError, "Strings must be maximum 100 characters long" if string_parameters.any?{ |s| s.length > 100 }
  end
end
