...
=end
    def self.validate_fizzbuzz(payload)
        ...
        return [false, "Integers must be 1 or greater"] if integer_parameters.any?{ |n| n.to_i < 1 }
        ...
        return [false, "Integers must be 1000000 or smaller"] if integer_parameters.any?{ |n| n.to_i > 1000000 }

        string_parameters = payload.values_at(:str1, :str2)

        return [false, "Strings must not be empty"] if string_parameters.any?{ |s| s.empty? }

        # Adding an arbitrary upper limit to string size
        return [false, "Strings must be maximum 100 characters long"] if string_parameters.any?{ |s| s.length > 100 }
